...
alloc_a_4k_page:
	...
	mov es, ax
	mov ax, SelectorFlatRW
	mov ds, ax				; use SelectorFlatRW to replace SelectorData

.search:
	bts [BitMap], eax		; bts judge the eax of bitmap is free or not
	jnc .find 				; if C = 0(no carry), jump to .find
	inc eax
	cmp eax, BitMapLen * 8	; judge over or not
	jl	.search				; if not, loop .search
	hlt						; finish
...
alloc_pages:
	...
	mov ecx, eax				; 4
	mov ebx, 4096
	mul ebx						; page number * page size

	mov ebx, [es:AvaLinearAddress] ; ebx -> linear address
	add [es:AvaLinearAddress], eax ; linear address update
...
.change_pte:
	...
	or eax, 0x00000007			; set 1
	mov [ebx], eax				; PTE point to the physical address
	add ebx, 4					; next PTE
	loop .change_pte
...
free_pages:
	...
	mov ebx, cr3
	and ebx, 0xfffff000
	and eax, 0xffc00000		; high 10 bit of linear address
	shr eax, 20
	add ebx, eax			; PDE item

	mov edx, [ebx]
	and edx, 0xfffffff8
	mov [ebx], edx			; set the low 3 bit zero and store back

	mov ebx, [ebx]			; now ebx means the first PTE item

	mov eax, [esp]			; eax = linear address
	add esp, 4
	and ebx, 0xfffff000
	and eax, 0x003ff000		; middle 10 bit
	shr eax, 10
	add ebx, eax			; PTE item
	
	mov ecx, [esp]			; ecx = ebx = 4 pages
	add esp, 4

.change_pte:				; unmap and set 0
	mov eax, [ebx]
	and eax, 0xfffffff8		; set 0
	mov edx, eax
	shr edx, 12				; bitmap position
	btr [BitMap], edx		; set 0
	mov [ebx], eax
	add ebx, 4				; next item
	loop .change_pte
...
_BitMap:	times	32	db	0xff	; low 1MB is occupied
