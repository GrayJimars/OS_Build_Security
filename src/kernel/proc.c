 ...
 * 
 *****************************************************************************/ 
void inqueue(struct proc* p) {
	assert(p->inqueue == 0);
	queue[p->whichqueue].proc_queue[queue[p->whichqueue].rear++] = p;
	queue[p->whichqueue].rear %= 50;
	queue[p->whichqueue].len++;
	p->inqueue = 1;
	p->isschedule = 1;
}
...
void outqueue(struct proc* p) {
	assert(p->inqueue);
	queue[p->whichqueue].front++;
	queue[p->whichqueue].front %= 50;
	queue[p->whichqueue].len--;
	p->inqueue = 0;
}
/* 
...
PUBLIC void schedule() {
	...
	while (!is_find) {
		...
		for (queue_num = 0; queue_num < 3; queue_num++) {
			...
			while (count > 0) {
				struct proc* p = queue[queue_num].proc_queue[pointer];
				if (p->p_flags != 0) {	// proc p has been blocked
					outqueue(p);
					inqueue(p);
				} else {	// proc p is runnable, then consider the following conditions
					if (p->ticks > 0 && p->time > 0) {	// p could be schedule, schedule it directly
						p_proc_ready = p;				// then set is_find flag to 1
						is_find = 1;					// then return
						break;
					} else if (p->ticks == 0) {			// p is over, outqueue, find next
						outqueue(p);
					} else if (p->time == 0) {			// p's time piece was used out
						outqueue(p);
						if (p->whichqueue < 2) {		// place it into next queue
							p->whichqueue++;
						}								// assign time piece
						p->time = queue[p->whichqueue].time_piece;
						inqueue(p);
					}
				}
		...
		// if there haven't proc to schedule, unlock those procs that 
